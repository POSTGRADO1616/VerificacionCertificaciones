...
@section('content')
    ...
    foreach ($cursos as $key => $value) {
        array_push($dataArray, [
            ...
            'Carrera' => $value->carrera->NombreCarrera, // Mostrar el nombre de la facultad
            'Facultad' => $value->facultad->NombreFacultad, // Mostrar el nombre de la facultad
            "<nobr> 
                <form action='" . route('Cursos.edit', ['Curso' => $value->id]) . "' method='GET'>
                    <button class='btn btn-xs btn-default text-teal mx-1 shadow' title='Details'>
                        <i class='fa fa-lg fa-fw fa-pen'></i>
                    </button>
                </form>"
                .
                "<form action='" . route('Cursos.destroy', ['Curso' => $value->id]) . "' method='POST'>
                " . csrf_field() . "
                " . method_field('DELETE') . "
